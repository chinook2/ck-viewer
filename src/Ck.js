...
Ext.apply(Ck, {
	...
	getPath: function() {
		...
		if(window.cordova){
			...
			path = Ext.manifest.profile + '/resources/ck-viewer';
		} else {
			// Standard web app need to add full url
			path = location.href + Ext.manifest.profile + '/resources/ck-viewer';
		}
	...
	getApi: function () {
		...
		/**
	 * Return the entiery manifest or one attribute of it
	 * @return {String/Object}
	 */
	getManifest: function(key) {
		if(Ext.isEmpty(key)) {
			return Ext.manifest;
		} else {
			return Ext.manifest[key];
		}
	},
	...
	getDefaultDirectory: function() {
		return "cdvfile://localhost/persistent/" + this.getManifest("name") + "/";
	},
	...
	openFile: function(opt) {
		...
		if(Ext.isEmpty(opt.mimeType)) {
			var extension = opt.path.split(".").pop();
			opt.mimeType = Ck.EXTENSION_MIMETYPE[extension];
		}
		...
		cordova.plugins.fileOpener2.open(
			...
			opt.mimeType, 
			{
				success : opt.onSuccess.bind(opt.scope),
