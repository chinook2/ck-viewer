...
Ext.apply(Ck, {
    ...
    /**
     * Get default resource path for a package, will always endswith '/', no need to add another one after.
    ...
    getPath: function (pkg) {
        ...
        if (this.getEnv() == 'development') {
            ...
            var result = Ck.getOption('inlineResources') || path;
        ...
        if (window.cordova) {
            ...
            var result = baseUrl + (Ext.manifest.profile || '') + resourcesUrl;
            return result + (result.endsWith("/") ? '' : '/');
        }
